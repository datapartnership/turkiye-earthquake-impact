...
p <- ggplot() +
  ...
  geom_sf(data = mi_sf[mi_sf$PARAMVALUE >= 6,],
          fill = "gray20",
          alpha = mi_sf$v_alpha[mi_sf$PARAMVALUE >= 6],
          color = NA) +
  theme_void() +
  labs(fill = "Change in\nNighttime\nLights") +
...
leaflet() %>%
  addTiles() %>%
  addPolygons(data = adm2_sf[adm2_sf$mi_max >= 5,]) %>%
  addPolygons(data = adm2_sf[adm2_sf$adm2_en %in% "GUROYMAK",], 
              color = "red")
